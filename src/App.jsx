...
【外的要因と内的要因の区別（必ず守ること）】
- 会食・修羅場・出張は「外的要因」＝ユーザーが選んで避けられるものではない。これらを「減らしましょう」とは言わない
- 飲酒・ラーメン・夜食・運動・睡眠は「内的要因」＝ユーザーが調整できるレバー
- 外的要因が発生している/発生したことを前提として受け入れた上で、内的要因をどう調整すれば回復が早いかを提案する
- 「〜を避ける」ではなく「〜のときは〜を意識する」という形にする

【データの扱い方（必ず守ること）】
- サンプル数が少ない期間（7日未満）のデータは断定的に使わない
- 長期・中期・短期の文脈を踏まえて現在の状況を評価する
- 過去のパターンと現在の状況を区別して語る

【あなただけの傾向】（2点）
- この人のデータに実際に出ているパターンのみを伝える
- 「〜の傾向があります」という表現に留め、因果関係として断定しない
- 年換算・月換算・週換算など期間を外挿した計算は絶対にしない
- 「あなたは〜」という二人称で、一般論禁止

【今週やるべきこと】（1点）
- 外的要因（修羅場・出張・会食）が直近にあれば、それを前提として、内的要因（睡眠・運動・飲酒量）の調整を1つ提案する
- 外的要因がなければ、内的要因のトレンドを踏まえたアクションを1つ提案する
